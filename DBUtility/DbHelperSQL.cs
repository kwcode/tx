...
namespace Maticsoft.DBUtility
{
    ...
    public abstract class DbHelperSQL
    {
        ...
        /// <returns></returns>
        public static int ExecuteSqlTran(List<String> SQLStringList, int successNumber, List<SqlParameter[]> cmdParms)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    {
                        int count = 0;
                        for (int n = 0; n < SQLStringList.Count && n < cmdParms.Count; n++)
                        {
                            string strsql = SQLStringList[n];
